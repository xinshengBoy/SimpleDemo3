...
import com.yks.simpledemo3.tools.SuitUtil;
...
public class SuitLines extends View {
    ...
    public SuitLines(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        basePadding = SuitUtil.dip2px(basePadding);
        ...
        coverLinePaint.setStrokeWidth(SuitUtil.dip2px(5));
        setLineStyle(SOLID);
        xyPaint.setTextSize(SuitUtil.size2sp(defaultXySize, getContext()));
        xyPaint.setColor(defaultXyColor);
        hintPaint.setTextSize(SuitUtil.size2sp(12, getContext()));
     ...
     * value：一组数据
     */
    private Map<Integer, List<SuitUnit>> datas = new HashMap<>();
    ...
    private int[] findSuitEdgeInVisual() {
        ...
        if (offset == 0) {// 不可滑动或当前位于最左边
            ...
            if (datas.get(0).size() > maxOfVisible) {
                ...
                List<SuitUnit> i = datas.get(0);
                while (low <= high) {
                    int mid = (low + high) >>> 1;
                    SuitUnit midVal = i.get(mid);
    ...
    private void drawLines(Canvas canvas, int startIndex, int endIndex) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
            for (int j = 0; j < datas.size(); j++) {
                SuitUnit current = datas.get(j).get(i);
                ...
                if (lineType == SEGMENT) {
                    ...
                    // 到这里肯定不是起始点，所以可以减1
                    SuitUnit previous = datas.get(j).get(i - 1);
    ...
    private void drawExsitDirectly(Canvas canvas) {
        ...
        for (int j = 0; j < datas.size(); j++) {
            if (!isLineFill() || !needCoverLine) {
                ...
                if (needCoverLine) {
                    coverLinePaint.setColor(SuitUtil.tryGetStartColorOfLinearGradient((LinearGradient) paints.get(j).getShader()));
                    canvas.save();
    ...
    private void drawClickHint(Canvas canvas) {
        SuitUnit cur = datas.get(clickIndexs[1]).get(clickIndexs[0]);
        ...
        canvas.drawText("y : " + cur.getValue(), bak.centerX(),
                bak.centerY() + 12 + SuitUtil.getTextHeight(hintPaint), hintPaint);
        hintPaint.setColor(hintColor);
    }
    ...
    private void drawX(Canvas canvas, int startIndex, int endIndex) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
            ...
            canvas.drawText(extX, datas.get(0).get(i).getXY().x, SuitUtil.calcTextSuitBaseY(xArea, xyPaint), xyPaint);
        ...
        if (yAreaBuffer == null) {
            ...
            for (int i = 0; i < countOfY; i++) {
                ...
                if (i == 0) {
                    ...
                    extY = minAndMaxOfY[1];
                    y = yRect.top + SuitUtil.getTextHeight(xyPaint) + 3;
                    yAxis = yRect.top;
                } else {
                    extY = minAndMaxOfY[0] + (minAndMaxOfY[1] - minAndMaxOfY[0]) / (countOfY - 1) * i;
                    y = yAxis = yRect.bottom - yRect.height() / (countOfY - 1) * i + SuitUtil.getTextHeight(xyPaint)/2;
                }
        ...
        if (yGridBuffer == null) {
            ...
            for (int i = 0; i < countOfY; i++) {
                ...
                if (i == 0) {
                    ...
                    yAxis = yRect.bottom - yRect.height() / (countOfY - 1) * i + SuitUtil.getTextHeight(xyPaint)/2;
                }
    ...
    private void feedInternal(Map<Integer, List<SuitUnit>> entry, List<Paint> entryPaints, boolean needAnim) {
        cancelAllAnims();
     ...
     * @param datas
     */
    private void calcMaxUnit(Map<Integer, List<SuitUnit>> datas) {
        // 先“扁平”
        List<SuitUnit> allUnits = new ArrayList<>();
        for (List<SuitUnit> line : datas.values()) {
            allUnits.addAll(line);
        }
        // 再拷贝，防止引用问题
        List<SuitUnit> bakUnits = new ArrayList<>();
        for (int i = 0; i < allUnits.size(); i++) {
            bakUnits.add(allUnits.get(i).clone());
        }
        // 最后排序，得到最大值
        Collections.sort(bakUnits);
        SuitUnit maxUnit = bakUnits.get(bakUnits.size() - 1);
        SuitUnit minUnit = bakUnits.get(0);
        minAndMaxOfY[0] = SuitUtil.getCeil5(Math.min(minUnit.getValue(), 0));
        minAndMaxOfY[1] = SuitUtil.getCeil5(Math.max(maxUnit.getValue(), 0));
    }
    ...
    private void calcAreas() {
        ...
        yArea = new RectF(validArea.left, validArea.top,
                ...
                validArea.bottom - SuitUtil.getTextHeight(xyPaint) - basePadding * 2);
    ...
    private long calcTotalCost() {
        ...
        return oneLineCost + oneLineCost / percentOfStartNextLineAnim * (datas.size() - 1) + SuitUnit.DURATION + 16;
    }
    ...
    private void showWithAnims() {
        ...
        for (int i = suitEdge[0]; i <= suitEdge[1]; i++) {
            for (List<SuitUnit> item : datas.values()) {
    ...
    private void startLineAnim(final int startIndex, final int endIndex) {
        final List<SuitUnit> line = datas.get(curAnimLine);
...
//    }
    ...
    public void setCoverLine(float withdp) {
        needCoverLine = true;
        coverLinePaint.setStrokeWidth(SuitUtil.dip2px(withdp) * 2);
        forceToDraw = true;
        postInvalidate();
    }
    ...
    public void setLineStyle(int style) {
        lineStyle = style;
        basePaint.setPathEffect(lineStyle == DASHED ? new DashPathEffect(new float[]{SuitUtil.dip2px(3),SuitUtil.dip2px(6)},0) : null);
        if (!datas.isEmpty()) {
            // 同时更新当前已存在的paint
            for (int i = 0; i < paints.size(); i++) {
                forceToDraw = true;
    ...
    public void setEdgeEffectColor(int color) {
        ...
        SuitUtil.trySetColorForEdgeEffect(edgeEffectLeft, edgeEffectColor);
        SuitUtil.trySetColorForEdgeEffect(edgeEffectRight, edgeEffectColor);
        postInvalidate();
    }
     ...
     * @param line
     */
    public void feedWithAnim(List<SuitUnit> line) {
        if (line == null || line.isEmpty()) return;
        final Map<Integer, List<SuitUnit>> entry = new HashMap<>();
        entry.put(0, line);
     ...
     * @param line
     */
    public void feed(List<SuitUnit> line) {
        if (line == null || line.isEmpty()) return;
        final Map<Integer, List<SuitUnit>> entry = new HashMap<>();
        entry.put(0, line);
        handler.post(new Runnable() {
    ...
    public static class LineBuilder {
        ...
        private Map<Integer, List<SuitUnit>> datas;
         ...
         * @return
         */
        public LineBuilder add(List<SuitUnit> data, int... color) {
